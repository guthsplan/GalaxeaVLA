#!/usr/bin/env python3
"""Gate A (CoT protocol v2): assert the real training token sequence for every behavior_cot_v2 builder.

v1 (BeliefGraph-conditioned) counterpart: tests/test_behavior_cot_template.py.

`tests/show_vla_label.py` renders one sample under one builder. This forces each
of the candidates in turn through the *real* dataset -> processor ->
InputPreprocessor.encode_train path and checks the nine invariants the CoT design
depends on:

    1. NO `BeliefGraph:` conditioning slot anywhere (the belief graph is an output-only
       CoT target; the conditioning region must equal the plain no-CoT prompt)
    2. the conditioning region is masked out of the LM loss
...
Usage:
    B1K_SUBSET_DIR=/path/to/labelled/subset \\
      python tests/test_behavior_cot_template.py --task behavior_cot_v2
"""
...
from g05.data_processor.processor.samples_builder import (  # noqa: E402
    BEHAVIOR_COT_V2_BUILDERS as BEHAVIOR_COT_BUILDERS,
)
from g05.models.g05.io.input_preprocessor import TOKEN_INDEX  # noqa: E402

IGNORE_INDEX = -100

#: builder class name -> (field to locate a sample by, semantic prefix expected in the CoT
#: region or None when the builder emits no CoT at all).
EXPECTED = {
    "AtomicTaskBaseSamplesBuilder": ("atomic_task", None),   # no CoT; Task: carries atomic_task
    "BBoxSubtaskCoTBuilder": ("bbox", "BBox"),
    "SubtaskCoTBuilder": ("atomic_task", "Subtask"),
    "BBoxCoTBuilder": ("bbox", "BBox"),
    "Trace2DCoTBuilder": ("trace_2d", "Trace"),
    "DeltaCoTBuilder": ("bg_delta", "Delta"),
    "UpdateCoTBuilder": ("bg_belief", "Belief"),
    "ObserveCoTBuilder": ("bg_observe", "Observe"),
    "EffectCoTBuilder": ("bg_effect", "Effect"),
    # multi-field spans: located by / checked on their FIRST field
    "ObserveUpdateCoTBuilder": ("bg_observe", "Observe"),
    "SubtaskEffectCoTBuilder": ("atomic_task", "Subtask"),
    "FullBeliefCoTBuilder": ("bg_observe", "Observe"),
}
...
def main() -> int:
    ...
    ap.add_argument("--task", default="behavior_cot_v2")
    ...
    for builder_cls in BEHAVIOR_COT_BUILDERS:
        ...
        if prefix is None:
            # no-CoT builder: the generative region starts at the literal "Action: <EOV>"
            cot_lo, cot_hi = (pred_runs[0] if pred_runs else (a0, a0))
        else:
        ...
        # 1 + 2: no BeliefGraph conditioning slot, and the conditioning region is masked
        has_bg_slot = "BeliefGraph:" in tok.decode(ids)
        if has_bg_slot:
            problems.append("a `BeliefGraph:` slot is present — the belief graph must be output-only")
        # every token up to the start of the generative region is conditioning
        if not bool((lab[:cot_lo] == IGNORE_INDEX).all()):
            problems.append("conditioning region is not fully masked")
        ...
        if prefix is not None:
            ...
            for pfx in ("Subtask:", "Delta:", "Belief:", "Observe:", "Effect:", "BBox:", "Trace:"):
                if pfx in tok.decode(ids[:a0]):
                    problems.append(f"no-CoT builder emitted a {pfx!r} span")
        ...
        print(f"  BeliefGraph  : present={has_bg_slot} expected=False  (conditioning masked: "
    ...
    if failures:
        ...
        for f in failures:
            print(f"  - {f}")
        return 1
    print(f"All {len(BEHAVIOR_COT_BUILDERS)} BEHAVIOR CoT builders satisfy the template invariants.")
    return 0
